    ...
    <>
      <div className="grid gap-4 lg:grid-cols-5">
        <section className="space-y-4 rounded-lg border border-border bg-surface p-4 lg:col-span-2">
          ...
          <button onClick={() => setShowHint((h) => Math.min(h + 1, exercise.hints.length))} className="rounded border border-border px-2 py-1 text-xs">
            Reveal Hint
          </button>
        ...
        <section className="rounded-lg border border-border bg-black p-4 lg:col-span-3 space-y-3">
          ...
          <button disabled={!completed} className="rounded bg-cyber/20 px-3 py-2 text-sm text-cyber disabled:bg-slate-700 disabled:text-slate-400">Next Lesson</button>
        </section>
      </div>
      <CheatSheetDrawer />
