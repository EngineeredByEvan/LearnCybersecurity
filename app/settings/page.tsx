...
export default function SettingsPage() {
  ...
  useEffect(() => { setApiKey(localStorage.getItem('anthropic_api_key') ?? ''); }, []);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded border border-border bg-surface p-4">
        ...
        <input value={apiKey} onChange={(e) => setApiKey(e.target.value)} className="w-full rounded border border-border bg-black p-2 text-xs" placeholder="sk-ant-..." />
        <button className="mt-2 rounded border border-border px-3 py-2" onClick={saveKey}>Save Key</button>
      </div>
      <div className="rounded border border-border bg-surface p-4">
